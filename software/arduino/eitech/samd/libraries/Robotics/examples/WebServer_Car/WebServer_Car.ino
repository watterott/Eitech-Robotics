...
#define IS_ACCESS_POINT 1 // 0 or 1
  ...
  char ssid[] = "robotics-car"; // AP name
...
char mdnsName[] = "robotics"; // MDNS name "robotics.local"
...
const uint8_t pinJ[8] = { RJ1, RJ1, RJ2, RJ3, RJ4, RJ5, RJ6, RJ7 };
      ...
      if(client.available()) 
      {
        ...
        if(c == '\n' && currentLineIsBlank) 
        {
          ...
          if(httpSplitRequest(httpRequest, httpRequestLen))
          {
            ...
            else if(!strcmp(httpRequestURL, "/background.jpg"))
            {
              sendBackgroundJPG(client);
            }
...
/*
 * send file background.jpg
 */
void sendBackgroundJPG(WiFiClient &client)
{
#include "background_jpg.h"  
  httpSendHeader(client, CT_JPEG);
